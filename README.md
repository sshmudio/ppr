...
- 🎯 Entry points to execute your program using `python -m <ppr>` or `$ ppr` with basic CLI argument parsing.
...
[![codecov](https://codecov.io/gh/sshmudio/ppr/branch/main/graph/badge.svg?token=ppr_token_here)](https://codecov.io/gh/sshmudio/ppr)
[![CI](https://github.com/sshmudio/ppr/actions/workflows/main.yml/badge.svg)](https://github.com/sshmudio/ppr/actions/workflows/main.yml)

Awesome ppr created by sshmudio

## Install

from source
```bash
git clone https://github.com/sshmudio/ppr ppr
cd ppr
make install
```
...
```bash
❯ ppr --help
Usage: ppr [OPTIONS] COMMAND [ARGS]...
...
```bash
❯ ppr create-user --help
Usage: ppr create-user [OPTIONS] USERNAME PASSWORD
...
```bash
ppr create-user admin admin --superuser
```
...
Run with `ppr run` and access http://127.0.0.1:8000/docs

![](https://raw.githubusercontent.com/rochacbruno/fastapi-project-template/master/docs/api.png)


**For some api calls you must authenticate** using the user created with `ppr create-user`.
...
-----------------------------------------------------
ppr/__init__.py              4      0   100%
ppr/app.py                  16      1    94%
ppr/cli.py                  21      0   100%
ppr/config.py                5      0   100%
ppr/db.py                   10      0   100%
ppr/models/__init__.py       0      0   100%
ppr/models/content.py       47      1    98%
ppr/routes/__init__.py      11      0   100%
ppr/routes/content.py       52     25    52%
ppr/routes/security.py      15      1    93%
ppr/routes/user.py          52     26    50%
ppr/security.py            103     12    88%
-----------------------------------------------------
TOTAL                               336     66    80%
...
```py
from ppr.config import settings
```
...
> `dynaconf_merge` is a boolean that tells if the settings should be merged with the default settings defined in ppr/default.toml.

### As environment variables
```bash
export ppr_KEY=value
export ppr_KEY="@int 42"
export ppr_KEY="@jinja {{ this.db.uri }}"
export ppr_DB__uri="@jinja {{ this.db.uri | replace('db', 'data') }}"
```
